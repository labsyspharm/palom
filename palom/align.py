...
class AlignmentAdaptor:
    ...
    def __init__(self, reader, channels, mode, level) -> None:
        self.reader = reader
    ...
    @property
    def thumbnail_downscale_factor(self):
        level = max(self.reader.level_downsamples.keys())
        return self.reader.level_downsamples[level]

    @property
    def thumbnail(self):
        if not self._thumbnail:
            self._thumbnail = self.reader.read_image(
...
class ReaderAlignerRefactor:
    ...
    def __init__(
        ...
        ref_thumbnail_down_factor=None,
        moving_thumbnail_down_factor=None
    ) -> None:
    ...
    def coarse_register_affin(self, n_keypoints=2000):
    ...
    def affine_transformed_moving_img(self):
...
class Transformer:
    ...
    def __init__(self, alignment_adaptor, block_affine_matrices) -> None:
        self.alignment_adaptor = alignment_adaptor
        self.block_affine_matrices = block_affine_matrices

        self.reader = self.alignment_adaptor.reader
        self.level = self.alignment_adaptor.level
